...
export default function Schedule() {
  const { data: allSchedules, isLoading } = useListSchedules({ query: { queryKey: getListSchedulesQueryKey() } });
  const published = allSchedules?.filter((s) => s.published) ?? [];

  const dates = ["ทั้งหมด", ...Array.from(new Set(published.map((s) => s.date)))];
  const [filterDate, setFilterDate] = useState("ทั้งหมด");

  const filtered = filterDate === "ทั้งหมด"
    ? published
    : published.filter((s) => s.date === filterDate);
          ...
          <>
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-24 glass rounded-2xl">
                ...
                <h3 className="text-xl font-medium text-white mb-2">ยังไม่มีตารางแข่งขัน</h3>
                <p className="text-muted-foreground">ติดตามการอัปเดตจากทีมงาน</p>
              </div>
            ) : (
